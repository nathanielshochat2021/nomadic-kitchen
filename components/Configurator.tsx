...
import { BASE_PRICE, formatPrice, GROUPS, PRESETS, summaryLines } from "@/lib/configurator";
...
export default function Configurator() {
  const { selection, total, toggleOption, isSelected, applyPreset, activePreset, reset } = useBuild();
  const params = useMemo(() => cartParams(selection), [selection]);
        ...
        <Reveal>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <p className="max-w-sm text-paper/70">
              Drag the cart to spin it. Every option you toggle reshapes it live —
              wood, finish, fridge, trailer and all.
            </p>
          </div>
        </Reveal>
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)]">
          ...
          <div className="space-y-8">
                      ...
                      >
                        ...
                        <span className="min-w-0 flex-1">
                          <span className="flex items-center justify-between gap-2">
                            <span className="truncate font-medium text-paper">{opt.label}</span>
                            <span className={`shrink-0 text-sm ${opt.price > 0 ? "text-ember-soft" : "text-paper/40"}`}>
                              {opt.price > 0 ? `+${formatPrice(opt.price)}` : "Incl."}
                            </span>
                          </span>
                          {opt.description && (
                            <span className="mt-0.5 block text-xs text-paper/55">{opt.description}</span>
                          )}
                        </span>
                        <span
            ...
            <p className="text-xs leading-relaxed text-paper/40">
              Prices shown are configuration estimates (Base build {formatPrice(BASE_PRICE)}). Final
              pricing is confirmed on your written quote. A deposit reserves your build slot.
            </p>
          </div>
        </div>
      </div>
